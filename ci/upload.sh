...
echo "============= PUSHING CF APP ===================================="
./cf push green --docker-image modveterans/forms:dev --docker-username $CF_DOCKER_USERNAME

echo "============= SET CF Environment Variables ======================"
./cf set-env $APP_NAME Email__ApiKey $NOTIFY_API
./cf set-env $APP_NAME Email__AFCSTemplateId $AFCS_TEMPLATE_ID
./cf set-env $APP_NAME Email__AFCSUserTemplateId $AFCS_USER_TEMPLATE_ID
./cf set-env $APP_NAME Email__AFCSEmailRecipient toby@codesure.co.uk
./cf set-env $APP_NAME Redis__Uri $REDIS_URL
